...
def init_complete_data():
    ...
    load_env()  # Cargar .env.dev por defecto

    database_url = os.getenv("DATABASE_URL")

    if not database_url:
        print(
            "❌ DATABASE_URL no encontrada; asegúrate de tener el archivo .env correcto o variables de entorno"
        )
        return
    # Crear engine similar a drop_postgresql_tables.py y abrir sesión
    ...
    try:
        with Session(engine) as db:
            ...
            categories_data = [
                ...
                CategoryCreate(
                    name="HOODIE", description="Hoodies / buzos con capucha"
                ),
