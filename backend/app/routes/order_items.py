...
def create_order_item(*, session: Session = Depends(get_session), order_item: OrderItem):
...
def read_order_item(*, session: Session = Depends(get_session), order_item_id: int):
...
def update_order_item(
    *, session: Session = Depends(get_session), order_item_id: int, order_item: OrderItem
):
...
def delete_order_item(*, session: Session = Depends(get_session), order_item_id: int):
    order_item = session.get(OrderItem, order_item_id)
    if not order_item:
        raise HTTPException(status_code=404, detail="Order item not found")
