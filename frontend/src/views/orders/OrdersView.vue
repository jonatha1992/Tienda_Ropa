    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:py-12 sm:px-6 lg:px-8">
      ...
      <div class="mb-8">
        ...
        <p class="mt-2 text-sm text-gray-600">
          AquÃ­ puedes ver el historial de todos tus pedidos
        </p>
      </div>
            ...
            <div class="border-t border-gray-200 pt-4 mt-4">
              ...
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center">
                  ...
                  <span class="text-sm font-medium text-gray-700">MÃ©todo de entrega:</span>
                </div>
              ...
              <div v-if="isShippingOrder(order.delivery_method || '')" class="space-y-3">
                ...
                <div v-if="order.tracking_number" class="flex items-center justify-between">
                  <div class="flex items-center">
                    ...
                    <span class="text-sm font-medium text-gray-700">NÃºmero de seguimiento:</span>
                  </div>
                  <div class="text-right">
                ...
                <div v-else-if="order.status === 'approved'" class="text-center py-3 bg-blue-50 rounded-lg">
                  ...
                  <p class="text-sm text-blue-700">Preparando envÃ­o</p>
                  ...
                  <div class="flex-1">
                    ...
                    <div v-if="order.status === 'approved'" class="space-y-2">
                      ...
                      <div class="bg-white rounded p-3 text-sm">
                        <div class="font-medium text-gray-900 mb-1">InformaciÃ³n de contacto:</div>
      ...
      <div v-else class="text-center py-12">
        ...
        <h3 class="text-lg font-medium text-gray-900 mb-2">No tienes pedidos aÃºn</h3>
        <p class="text-gray-600 mb-6">Cuando realices tu primera compra, aparecerÃ¡ aquÃ­</p>
        <div class="space-x-4">
          <router-link
            to="/shop"
...
const loadOrders = async () => {
  try {
    ...
    console.log('ðŸ“§ Email del usuario:', authStore.backendUser?.email);
    
    orders.value = await ordersApi.getMyOrders();
    console.log('âœ… Pedidos cargados:', orders.value);
    console.log('ðŸ“Š Cantidad de pedidos:', orders.value.length);
    ...
    if (error?.response?.status === 401) {
      console.log('ðŸ” Error 401: SesiÃ³n expirada');
      toast.error('SesiÃ³n expirada. Por favor, inicia sesiÃ³n nuevamente.');
      authStore.logout();
      router.push('/auth');
    } else if (error?.response?.status === 400) {
      console.log('âš ï¸ Error 400: Problema con la solicitud');
      toast.error('Error en la solicitud. Intenta cerrar sesiÃ³n y volver a iniciar.');
    } else if (error?.response?.status === 404) {
      console.log('ðŸ” Error 404: Endpoint no encontrado');
      toast.error('Error del servidor. El servicio no estÃ¡ disponible.');
    } else if (error?.response?.status >= 500) {
      console.log('ðŸ”¥ Error del servidor:', error?.response?.status);
      toast.error('Error del servidor. Por favor, intenta mÃ¡s tarde.');
    } else if (error?.code === 'NETWORK_ERROR' || error?.message?.includes('Network Error')) {
      console.log('ðŸŒ Error de red');
...
const trackShipment = (trackingNumber: string | undefined, provider: string | undefined) => {
  if (!trackingNumber || !provider) {
    toast.error('InformaciÃ³n de tracking no disponible');
    return;
  }
