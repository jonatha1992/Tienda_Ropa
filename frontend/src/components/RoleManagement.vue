<template>
  <div class="max-w-6xl mx-auto p-6">
    <div class="mb-6">
      <h1 class="font-heading text-3xl font-bold text-gray-900 mb-2">Gestión de Roles</h1>
      <p class="font-body text-gray-600">Administra los roles y permisos de usuarios</p>
    </div>
    ...
    <div class="mb-6 flex gap-4">
      ...
      >
        <span class="font-body">{{ loading ? 'Cargando...' : 'Inicializar Roles por Defecto' }}</span>
      </button>
      ...
      >
        <span class="font-body">Crear Nuevo Rol</span>
      </button>
    </div>

    <!-- Create Role Form -->
    <div v-if="showCreateForm" class="mb-6 bg-white p-6 rounded-lg shadow-md border">
      <h3 class="font-heading text-lg font-semibold mb-4">Crear Nuevo Rol</h3>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label class="font-body block text-sm font-medium text-gray-700 mb-2">Nombre del Rol</label>
          <select v-model="newRole.name" class="w-full border border-gray-300 rounded-md px-3 py-2">
            <option class="font-body" value="">Seleccionar tipo</option>
            <option class="font-body" value="admin">Admin</option>
            <option class="font-body" value="manager">Manager</option>
            <option class="font-body" value="employee">Employee</option>
            <option class="font-body" value="user">User</option>
          </select>
        </div>
        <div>
          <label class="font-body block text-sm font-medium text-gray-700 mb-2">Descripción</label>
          <input
      ...
      <div class="mt-4 flex gap-2">
        <button
          ...
          class="bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white px-4 py-2 rounded-md"
        >
          <span class="font-body">Crear Rol</span>
        </button>
        <button
          @click="cancelCreate"
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="font-heading text-xl font-semibold text-gray-900">Roles Existentes</h2>
      </div>
      
      <div v-if="loading && roles.length === 0" class="font-body p-6 text-center text-gray-500">
        Cargando roles...
      </div>
      
      <div v-else-if="roles.length === 0" class="font-body p-6 text-center text-gray-500">
      ...
      <div v-else class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th class="font-heading px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID</th>
              <th class="font-heading px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nombre</th>
              <th class="font-heading px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Descripción</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="role in roles" :key="role.id">
              <td class="font-body px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ role.id }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="font-body inline-flex px-2 py-1 text-xs font-semibold rounded-full"
                      :class="getRoleBadgeClass(role.name)">
                  {{ role.name.toUpperCase() }}
                </span>
              </td>
              <td class="font-body px-6 py-4 text-sm text-gray-900">{{ role.description || 'Sin descripción' }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="font-body inline-flex px-2 py-1 text-xs font-semibold rounded-full"
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                ...
                >
                  <span class="font-body">Ver Usuarios</span>
                </button>
                <button
                ...
                >
                  <span class="font-body">{{ role.is_active ? 'Desactivar' : 'Activar' }}</span>
                </button>
              </td>
            </tr>
          </tbody>
    ...
    <div v-if="showUsersModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
        <div class="mt-3">
          ...
          <div v-if="usersWithRole.length === 0" class="font-body text-center text-gray-500 py-8">
          ...
          <div v-else class="space-y-2 max-h-60 overflow-y-auto">
            ...
            >
              <div>
                <div class="font-body font-medium text-gray-900">{{ user.email }}</div>
              ...
              >
                <span class="font-body">Remover Rol</span>
              </button>
            </div>
          </div>
        </div>
...
import { rolesApi } from '../config/index'
