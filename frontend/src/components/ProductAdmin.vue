<template>
  <div class="container p-4 mx-auto">
    ...
    <!-- Formulario -->
    <form @submit.prevent="confirmSave" class="p-6 mb-8 bg-white border rounded-lg shadow-md">
      ...
      <div class="mt-6">
        ...
        <div v-for="(variant, index) in product.variants" :key="index"
          ...
          <input type="text" v-model="variant.color" placeholder="Color" class="border-gray-300 rounded-md shadow-sm">
          <input type="text" v-model="variant.talle" placeholder="Talle" class="border-gray-300 rounded-md shadow-sm">
          <input type="number" v-model.number="variant.stock" placeholder="Stock"
            class="border-gray-300 rounded-md shadow-sm">
          <button type="button" @click="removeVariant(index)" class="text-red-500 hover:text-red-700">Eliminar</button>
        </div>
        ...
        <tbody>
          <tr v-for="p in products" :key="p.id">
            ...
            <td class="px-4 py-2 border-b">
              ...
              <button @click="confirmDelete(p.id)" class="ml-4 text-red-600 hover:text-red-900">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
...
async function fetchProducts() {
  ...
  try {
    const response = await fetch(API_URL, {
      headers: { 'Authorization': 'Bearer fake-token' }, // Reemplazar con token real
    });
...
async function saveProduct() {
  ...
  const url = editing.value ? `${API_URL}${products.value.find(p => p.name === product.value.name)?.id}` : API_URL;
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'Authorization': 'Bearer fake-token', // Reemplazar con token real
      },
